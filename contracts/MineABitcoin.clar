  ...
  (let
    ...
    (if (and (is-eq (var-get game-state) STATE-ONGOING) (>= elapsed effective-duration))
      (begin
        ...
        ;; Cooldown starts at the block the round should have ended
  ...
  (let
    ...
    (let
      ...
      (let
        ...
        ;; Pay from vault
        (try! (vault-withdraw token caller reward))

        ;; Track shares paid (rolling windows + all-time)
        (record-shares-paid reward)

        ;; Track total withdrawn per user
        (map-set total-withdrawn caller
          (+ (default-to u0 (map-get? total-withdrawn caller)) reward)
        )

        ;; Snapshot current mask, clear pending
        (map-set miner-state { miner: caller, round: round }
...
(define-public (start-new-round)
  (begin
    ...
    (asserts! (is-eq (var-get game-state) STATE-COOLDOWN) ERR-GAME-NOT-COOLDOWN)
    ;; Must wait cooldown-blocks after cooldown started
    (asserts! (>= burn-block-height (+ (var-get cooldown-start-height) (var-get cooldown-blocks))) ERR-GAME-NOT-COOLDOWN)

    ;; Snapshot the final global mask for the ending round (enables cross-round claims)
    (map-set round-final-mask (var-get current-round) (var-get global-mask))

    ;; Reset for new round
    (var-set current-round (+ (var-get current-round) u1))
    (var-set game-state STATE-ONGOING)
    ;; New round starts at current Bitcoin block height
    ...
    ;; Carry over next-round pool into mining reward
